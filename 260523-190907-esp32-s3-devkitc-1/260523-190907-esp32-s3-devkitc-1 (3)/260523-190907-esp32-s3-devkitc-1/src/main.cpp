 ...
 *   2. 飞机大战射击游戏：移动射击，敌人递增难度
 *   3. 学习伴侣（番茄钟）：4 种时长选择，倒计时，鼓励提醒
 *
 * 【程序架构】
 * main.cpp (主控)
 *   ├── 初始化 setup()
 *   │   ├── initAudio()       → 配置 PWM 蜂鸣器
 *   │   ├── initKeyboard()    → 配置矩阵键盘
 *   │   ├── initLCD()         → 配置 TFT LCD
 *   │   └── initOLED()        → 配置 OLED
 *   └── 主循环 loop()
 *       ├── scanKeyboard()    → 读取当前按键
 *       └── switch(currentMode)
 *           ├── MODE_MENU  → handleMenu() / 超时检测 / OLED 刷新
 *           ├── MODE_PIANO → pianoGameLoop()
 *           ├── MODE_PLANE → planeGameLoop()
 *           └── MODE_STUDY → studyBuddyLoop()
 *
 * 【程序执行流程】
...
void loop() {
  ...
  while (keyQueueAvailable()) {
    ...
    switch (currentMode) {
      ...
      case MODE_PLANE:  planeGameLoop(key);  break;
